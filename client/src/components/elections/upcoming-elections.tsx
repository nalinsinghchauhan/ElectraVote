...
export default function UpcomingElections({ 
  ...
  const { data: elections, isLoading, error } = useQuery<ElectionWithCandidates[]>({
    queryKey: ["/api/elections/status/upcoming"],
  });
  ...
  if (!elections || elections.length === 0) {
    return (
      <div className="text-center py-12 bg-card rounded-lg shadow-md border border-border/40">
        ...
        <h3 className="mt-2 text-sm font-medium text-foreground">No upcoming elections</h3>
        <p className="mt-1 text-sm text-muted-foreground">There are no elections scheduled for the future.</p>
        {isAdmin && onCreateElection && (
          <div className="mt-6">
            <Button onClick={onCreateElection} className="bg-primary/90 hover:bg-primary">Schedule New Election</Button>
          </div>
        )}
      </div>
    );
